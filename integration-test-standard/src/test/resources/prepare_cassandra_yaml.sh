...
sed -i 's/^\(authorizer: \).*/\1com.ericsson.bss.cassandra.ecaudit.auth.AuditAuthorizer/' cassandra.yaml
...
sed -i 's/^num_tokens:.*/num_tokens: 1/' cassandra.yaml
sed -i 's/^enable_user_defined_functions:.*/enable_user_defined_functions: true/' cassandra.yaml
